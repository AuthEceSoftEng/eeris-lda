	...
	public boolean init(LDACmdOption option){
		...
		if (option.est){
			if (!trnModel.initNewModel(option))
				return false;
			//-----HARD-CODED-----//
			trnModel.data.localDict.writeWordMap("output\\"+trnModel.K+"_"+trnModel.alpha+"_"+trnModel.beta+ File.separator + option.wordMapFileName);
		...
		for (trnModel.liter = lastIter + 1; trnModel.liter < trnModel.niters + lastIter; trnModel.liter++){
			...
			if (option.savestep > 0){
				if (trnModel.liter % option.savestep == 0){
					...
					computeTheta();
					computePhi();
					trnModel.computePerplexity("estimate");
