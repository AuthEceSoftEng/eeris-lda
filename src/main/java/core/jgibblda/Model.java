...
public class Model {	
	...
	public double perplexity; // (NEW) perplexity of the model on the dataset
	...
	public List<Map<String, Double>>  topicWordProbList;
	public String[][] wordClouds;
	...
	public boolean saveModelTwords(String filename){
		...
		try{
			...
			wordClouds = new String[K][twords];
			topicWordProbList = new ArrayList<Map<String, Double>>();
			
			for (int k = 0; k < K; k++){				
				Map<String, Double> topWordMap = new HashMap<String, Double>(); 
				...
				for (int i = 0; i < twords; i++){
					if (data.localDict.contains((Integer)wordsProbsList.get(i).first)){
						...
						topWordMap.put(word,  (Double) wordsProbsList.get(i).second);
						//System.out.println("Word: "+word+", probability: "+(Double) wordsProbsList.get(i).second);
						wordClouds[k][i]=word;
						//writer.write("\t" + word + " " + wordsProbsList.get(i).second + "\n");
					}
				}
				topicWordProbList.add(topWordMap);
			} //end foreach topic			
	...
	public boolean saveModel(String modelName){
		/**Modified-Save the model to the output-specific directory**/
		//------HARDCODED PATH saveDir------//
		String saveDir = "output\\"+Integer.toString(K)+"_"+Double.toString(alpha)+"_"+Double.toString(beta);
		if (!saveModelTAssign(saveDir + File.separator + modelName + tassignSuffix)){
			return false;
		}
		
		if (!saveModelOthers(saveDir + File.separator + modelName + othersSuffix)){			
			return false;
		}
		
		if (!saveModelTheta(saveDir + File.separator + modelName + thetaSuffix)){
			return false;
		}
		
		if (!saveModelPhi(saveDir + File.separator + modelName + phiSuffix)){
			return false;
		}
		
		if (twords > 0){
			if (!saveModelTwords(saveDir + File.separator + modelName + twordsSuffix))
				return false;
		}
		return true;
	}
	...
	public boolean initNewModel(LDACmdOption option, Model trnModel){
		if (!init(option))
			return false;
		//HARD-CODED
		LDADataset dataset = LDADataset.readDataSet("dataset\\test" + File.separator + dfile, trnModel.data.localDict);
		if (dataset == null){
	...
	public void computePerplexity(String dataType) {
		
		int totalTerms = 0;
		double logLikelihood = 0;
		
		for (int d = 0; d < M; d++) { // for each document
			Document document = data.docs[d];
			totalTerms += document.words.length;
			for (int n = 0; n < document.words.length; n++) { // for each term/word
				int w = document.words[n];
				double sum = 0;
				for (int k = 0; k < K; k++) { // for each topic
					//System.out.println("Document: "+d+", Topic: "+k+", Word id: "+w);
					sum += theta[d][k]*phi[k][w];
				} // end for each topic
				logLikelihood += Math.log(sum);
			} // end for each word
		} // end for each document
		
		perplexity = Math.exp(-logLikelihood/totalTerms);

		// Save model's values to external files
		exportPerplexity(dataType, logLikelihood);
		
	}
	
	public static double logSum(double[] val) {
		...
		double logArg = 0;
		for (int i = 0; i < val.length; i++) {
			logArg += Math.exp(val[i] - valMax);
		}
		
		return valMax + Math.log(logArg);
	}
	
	private void exportPerplexity(String dataType, double ll) {
		
		try(FileWriter fw = new FileWriter("perplexity.csv", true);
				BufferedWriter bw = new BufferedWriter(fw);
			    PrintWriter out = new PrintWriter(bw))
			{
			    out.println(dataType+","+K+","+alpha+","+beta+","+liter+","+ll+","+perplexity);
			} catch (IOException e) {
